...
/*
	...
	*/
	...
	snprintf(pkt_json, sizeof(pkt_json),
		"{"
		"\"type\":\"packet\","
		"\"timestamp\":\"%s\","
		"\"src\":%u,"
		...
		"\"length\":%u"
		"}",
		header_values[4],	// Priority
		header_values[1], 	// Source address
		header_values[0], 	// Destination address
		header_values[2], 	// Source port
		header_values[3], 	// Destination port
		packet->length	 	// Length of the data
	);
	status_publisher_send(pkt_json);
